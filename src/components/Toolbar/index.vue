...
import { saveflowData } from "@/assets/api/flow";
      ...
      editorData.edges.some(attrs => {
        ...
        if (terms.length >= 1) {
          if (isObjHasNull(terms[0])) {
            this.$message({
              message:`请完整设置节点[${attrs.source}]至节点[${attrs.target}]的条件`,
              showClose: true, type: "warning", duration: 1500
            });
            return true;
          }
        }
